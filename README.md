...
支持从列表页逐门处理状态为“未填写”的课程。每门预填后会停下，由你检查、手动保存并返回列表。**不会点击“保存”或最终“提交评价”。**
...
5. 点击页面右下角“教评辅助”。
6. Lite：检查填写结果后手动保存。
7. Queue：每门预填完成后，手动保存并返回列表；全部处理完后，再检查列表并自行决定是否最终提交评价。
