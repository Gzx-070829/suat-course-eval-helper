...
5. 确认后，Queue 会进入第一门课程并填写评分和文字，然后停下等待检查。
6. 由你手动保存并返回列表后，Queue 会重新扫描下一门“未填写”课程，不依赖固定行号。
7. 全部处理结束后，检查列表和每门课程内容，再手动决定是否点击最终“提交评价”。

### 在详情页单独使用 Queue

在详情页点击“教评辅助”，Queue 只填写当前课程；检查后由你手动保存。
...
最终提交会让整批评价正式生效。预填评分和文字模板不一定符合每门课程的真实体验，因此必须由用户检查后亲自决定，脚本不会点击任何最终提交按钮。
